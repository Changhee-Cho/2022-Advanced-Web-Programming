	...
	<select name=choose>
		...
		<tr>
			...
			<th>조회수</th>
					...
					if($_POST[choose]=="제목")
						$result=mysqli_query($db, "SELECT * FROM p_library WHERE subject like '%{$_POST[query]}%' order by num desc");
					else
						$result=mysqli_query($db, "SELECT * FROM p_library WHERE name like '%{$_POST[query]}%' order by num desc");
					$i=0;
					$j=mysqli_num_rows($result);
					while($row=mysqli_fetch_array($result)){
						$k=$j-$i;
						echo("<tr align=center><th>$k</th><th><a href=library_read.php?id={$row[num]}>{$row[subject]}");
