...
echo '
		<div class="contents_box">
			 ...
			 <h1 class="nt_1">공지사항</h1><hr>';   
			
			if($_SESSION[admin]==1)
					echo '<div class="nt_2"><a href=notice_write.php><input type="button" value="글쓰기" class="nt_10"></a></div>';
   			   echo '
                   <form method="POST" action="notice_search.php" class="nt_3">
	<select name=choose>
		<option>제목</option>
		<option>올린이</option>
	</select>
		...
		<tr>
			...
			<th>조회수</th>
					...
					if($_POST[choose]=="제목")
						$result=mysqli_query($db, "SELECT * FROM p_notice WHERE subject like '%{$_POST[query]}%' order by num desc");
					else
						$result=mysqli_query($db, "SELECT * FROM p_notice WHERE name like '%{$_POST[query]}%' order by num desc");
					$i=0;
					$j=mysqli_num_rows($result);
					while($row=mysqli_fetch_array($result)){
						$k=$j-$i;
						echo("<tr align=center><th>$k</th><th><a href=notice_read.php?id={$row[num]}>{$row[subject]}");
