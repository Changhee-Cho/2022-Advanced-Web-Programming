	...
	<select name=choose>
		...
		<tr>
			...
			<th>조회수</th>
					...
					$result=mysqli_query($db, "select * from p_notice order by num desc");
					$i=0;
					$j=mysqli_num_rows($result);
					while($row=mysqli_fetch_array($result)){
						$k=$j-$i;
						echo("<tr align=center><th>$k</th><th class='nt_tb1'><a href=notice_read.php?id={$row[num]}>{$row[subject]}");
